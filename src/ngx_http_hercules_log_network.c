...
static void ngx_http_hercules_chunk_timeout(ngx_event_t* ev);
static inline void ngx_http_hercules_free_chunk(ngx_http_hercules_chunk_t* chunk, ngx_pool_t* pool);
...
void ngx_http_hercules_send_metrics(ngx_http_hercules_main_conf_t* conf){
    ...
    if(buffer_size > 0){
        ...
        chunk->status = NGX_HERCULES_CHUNK_NULL;
        chunk->pool = pool;
        chunk->event = ngx_pcalloc(pool, sizeof(ngx_event_t));
        if(chunk->event == NULL){
            ngx_pfree(pool, buffer->start);
            ngx_pfree(pool, buffer);
        ...
        if(last_task == NULL){
            ngx_pfree(pool, buffer->start);
            ngx_pfree(pool, buffer);
            ngx_pfree(pool, chunk->event);
            ngx_pfree(pool, chunk);
            return;
        }
        last_task->chunk = chunk;
        chunk->queue_task = last_task;

        ngx_event_add_timer(chunk->event, HERCULES_TIMEOUT);
...
static void ngx_http_hercules_chunk_timeout(ngx_event_t* ev){
    ngx_http_hercules_chunk_t* chunk = (ngx_http_hercules_chunk_t*) ev->data;
    ngx_pool_t* pool = chunk->pool;

    if(chunk->status == NGX_HERCULES_CHUNK_NULL){
        /* Drop chunk */
        ngx_http_hercules_free_chunk(chunk, pool);
    } else {
        /* Try later */
        ngx_event_add_timer(ev, HERCULES_TIMEOUT_RETRIES);
    }
}

static inline void ngx_http_hercules_free_chunk(ngx_http_hercules_chunk_t* chunk, ngx_pool_t* pool){
    ngx_http_hercules_queue_task_t* q_task;
    ngx_queue_t* q;

    if(chunk->event->timer_set){
        ngx_event_del_timer(chunk->event);
    }

    ngx_pfree(pool, chunk->buffer->start);
    ngx_pfree(pool, chunk->buffer);
    ngx_pfree(pool, chunk->event);
    if(chunk->queue_task != NULL){
        q_task = chunk->queue_task;
        q = &q_task->queue;
        ngx_queue_remove(q);
...
static void ngx_http_hercules_send_chunk(ngx_http_hercules_ctx_t* ctx){
    ...
    ctx->active_chunk->queue_task = NULL;
    ctx->active_chunk->status = NGX_HERCULES_CHUNK_ON_WRITE;
    ngx_queue_remove(q);
    ngx_pfree(pool, q_task);
...
static void ngx_http_hercules_read_handler(ngx_event_t *rev){
    ...
    if (ctx->active_chunk == NULL || ctx->active_chunk->status == NGX_HERCULES_CHUNK_NULL){
        if (rev->timer_set) {
            ngx_del_timer(rev);
        }
        return;
    ...
    if (ctx->active_chunk->status == NGX_HERCULES_CHUNK_ON_WRITE){
        if (rev->timer_set) {
            ngx_del_timer(rev);
        }
        goto error;
    ...
    ngx_http_hercules_free_chunk(ctx->active_chunk, ctx->pool);
...
static void ngx_http_hercules_write_handler(ngx_event_t *wev){
    ...
    if (sended_size > 0){
        ...
        if ((size_t) sended_size == buffer_size){
            ...
            ctx->active_chunk->status = NGX_HERCULES_CHUNK_ON_READ;
...
static inline void ngx_http_hercules_error(ngx_http_hercules_ctx_t* ctx){
    ctx->active_chunk->status = NGX_HERCULES_CHUNK_NULL;
    ...
    if(last_task == NULL){
        ...
        ctx->active_chunk->queue_task = last_task;
